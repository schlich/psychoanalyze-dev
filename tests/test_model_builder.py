...
def test_model_builder_lifecycle(linear_model_class, tmp_path):
    X = pd.DataFrame({"x": np.linspace(0, 10, 20)})
    y = 2 * X["x"] + 1 + np.random.normal(0, 0.5, 20)

    model = linear_model_class()
    idata = model.fit(X, y)
    ...
    loaded_model = linear_model_class.load(str(save_path))
